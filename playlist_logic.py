...
def compute_playlist_stats(playlists: PlaylistMap) -> Dict[str, object]:
    ...
    total = len(all_songs) # original : total = len(hype)
...
def search_songs(
    ...
    for song in songs:
        value = str(song.get(field, "")).lower() # gets all songs
        if value and q in value: # original - if value and value in q:
...
def lucky_pick(
    ...
    if mode == "hype":
        ...
        songs = playlists.get("Hype", []) + playlists.get("Chill", []) + playlists.get("Mixed", []) # orig excluded "Mixed"
